...
/*   Updated: 2016/04/20 18:49:43 by cledant          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

size_t			ft_printf_parse_type(char *format, va_list list_arg, int *skip)
{
	char	*parse;
	size_t	ret;
	t_flags	flag;
...
//	ft_putendl(format);
	if ((parse = ft_printf_analyse_string(format + 1)) != NULL)
	{
		...
		ret = ft_printf_print_args(flag, list_arg);
		if (ft_strcmp(parse, "%") == 0)
			*skip = 1;
		ft_strdel(&parse);
		ft_printf_del_flags(&flags);
		return (ret);
	}
	return (0);
}
